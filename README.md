...
# run app
java -jar target/java-rothschild-eleven-1.0-SNAPSHOT.jar
...
docker buildx build   --platform linux/amd64   -t parsontodd/java-rothschild-eleven:latest   --push .
```
...
```bash
# namespace (templated)
kubectl get ns kaiser >/dev/null 2>&1 || kubectl create ns kaiser
...
# watch rollout
kubectl rollout status deploy/java-rothschild-eleven -n kaiser

# get external endpoint
kubectl get svc java-rothschild-eleven-svc -n kaiser
```
...
```bash
kubectl port-forward svc/java-rothschild-eleven-svc 8080:80 -n kaiser
...
kubectl rollout restart deploy/java-rothschild-eleven -n kaiser
kubectl rollout status deploy/java-rothschild-eleven -n kaiser
```
...
info.app.name=javarothschildeleven
...
├─ src/
│  ├─ main/
│  │  ├─ java/com/harness/javarothschildeleven/
│  │  │  ├─ App.java
...
│  │        └─ index.html
│  └─ test/
│     └─ java/com/harness/javarothschildeleven/AppTest.java
├─ kubernetes/
│  ├─ deployment.yml
│  └─ service.yml
├─ Dockerfile
...
© dustin.rothschild@harness.io. For demo/PoV purposes.
